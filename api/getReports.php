...
{ 
    ...
    $survey_list = $db->get('survey', null, $cols);
...
if($_GET['identifier'] === 'last_30_days' || $_GET['identifier'] === 'last_month' || $_GET['identifier'] === 'this_month'|| $_GET['identifier'] === 'custom_range')
{
    ...
    {
       // echo 'i m here';
       $firstChartData= $functionsList->getLast3MonthsData_analytics($_GET['survey_id'], "responses", $_GET['area_id'],$_GET['start_date'], $_GET['end_date']);
    }
    else{
        //	echo 'i m here';
		$firstChartData=$functionsList->getLast1MonthsData_analytics($_GET['survey_id'], "responses", $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);
	}
}
elseif($_GET['identifier'] === 'Yesterday' || $_GET['identifier'] === 'Today'){
	$firstChartData = $functionsList->getOneDayData_analytics($_GET['survey_id'], "responses", $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);
}
elseif ($_POST['identifier']=== '7_days') {
 $firstChartData=$functionsList->getSevenDaysRatingData($_GET['survey_id'], "responses", $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);
}

$a['ratingBasedOnOptions'] = $functionsList->getRatingBasedOnOptions($_GET['survey_id'], $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);

//$area = $functionsList->getRatingBasedOnArea($_GET['survey_id'], $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);

$a['ratingBasedOnArea'] = $functionsList->getRatingBasedOnArea($_GET['survey_id'], $_GET['area_id'], $_GET['start_date'], $_GET['end_date']);
