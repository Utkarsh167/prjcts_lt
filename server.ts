...
import { logger, RedisClient } from "@lib/index";
...
try {
	...
	const init = async () => {
		...
		try {
			...
			await server.start();
			// console.log("==============", "Testttt loaded1");
			let db = new Database();

			await db.connectToDb();
			// console.log("==============", "Testttt loaded3");
			...
			console.info(`Hapi server listening on ${config.SERVER.IP}:${config.SERVER.PORT}, in ${config.SERVER.TAG} mode`);
