...
let userType = Joi.number().optional().description("1=Employee, 2= Driver");
...
let documents = Joi.array().optional().items({
	...
	// optional field -- satyam
	frontImageUrl: Joi.string().optional().allow("").label("Front image url"),
...
let tripStatus = Joi.number().allow("").optional().valid([
	...
	// all trip status -- satyam
	config.CONSTANT.TRIP_STATUS.ALL_TRIP,
...
// optional field -- satyam
let dlRenewalDate = Joi.optional().allow("").description("dlRenewalDate");
let DlBadgeNO = Joi.string().optional().allow("").description("DlBadgeNO");
// CAb module
...
// satyam - allow cabImage - null
// let cabImage = Joi.string().optional().description("cabImage");
let cabImage = Joi.string().optional().allow("").description("cabImage");
