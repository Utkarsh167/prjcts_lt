			...
			validate: async (request: Hapi.Request, accessToken: string, h: Hapi.ResponseToolkit) => {
				try {
					...
					if (!isValid) {
						...
						if (loginHistoryData.length === 0 && tokenData.adminType === config.CONSTANT.ADMIN_TYPE.SUB_ADMIN) {
							throw customException.permissionChangedByAdmin();
						} else if (loginHistoryData.length === 0 && tokenData.adminType === config.CONSTANT.ADMIN_TYPE.ADMIN) {
