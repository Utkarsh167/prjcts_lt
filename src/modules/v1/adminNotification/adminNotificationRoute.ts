...
export let adminNotificationRoute = [
	{
		method: "POST",
		path: `${config.SERVER.API_BASE_URL}/superAdmin-notification`,
		handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
			...
			try {
				let result = await adminNotificationController.sendAddNotification({ ...tokenData, ...payload });
	...
	{
		method: "DELETE",
		path: `${config.SERVER.API_BASE_URL}/superAdmin-notification/{notificationId}`,
		handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
			let tokenData: TokenData = request.auth.credentials.tokenData;
	...
	{
		method: "GET",
		path: `${config.SERVER.API_BASE_URL}/superAdmin-notification/details`,
		handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
	...
	// {
	// 	method: "POST",
	// 	path: `${config.SERVER.API_BASE_URL}/superAdmin-notification/send/{userId}`,
	// 	handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
	// 		let tokenData: TokenData = request.auth.credentials.tokenData;
	...
	{
		method: "POST",
		path: `${config.SERVER.API_BASE_URL}/superAdmin-notification/resend`,
		handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
			let tokenData: TokenData = request.auth.credentials.tokenData;
			// let params: NotificationId = request.params;
			let payload: AddAdminNotificationRequest = request.payload;
			try {
				let result = await adminNotificationController.resendUpdateNotification({ ...tokenData, ...payload });
				return responseHandler.sendSuccess(h, result);
			} catch (error) {
				return responseHandler.sendError(error);
			}
		...
		config: {
			...
			validate: {
				headers: commonValidator.adminAuthorizationHeaderObj,
				payload: adminNotificationValidator.validateResendNotification,
				failAction: appUtils.failActionFunction
			},
			plugins: {
				"hapi-swagger": {
	...
	{
		method: "GET",
		path: `${config.SERVER.API_BASE_URL}/superAdmin/admin-list`,
		handler: async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
			let tokenData: TokenData = request.auth.credentials.tokenData;
			let query: ListingRequest = request.query;
			try {
				let result = await adminNotificationController.adminList({ ...tokenData, ...query });
				return responseHandler.sendSuccess(h, result);
			} catch (error) {
				return responseHandler.sendError(error);
			}
