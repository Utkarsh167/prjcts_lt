...
export class AdminNotificationDao extends BaseDao {
	...
	async notificationList(params: ListingRequest) {
		try {
			let { pageNo, limit, searchKey, sortBy, fromDate, toDate, scheduleFromDate, scheduleToDate, companyCode } = params;
			let aggPipe = [];
			let match: any = {};
			if (params.adminType === config.CONSTANT.ADMIN_TYPE.ADMIN) {
				match = { "$and": [{ "adminType": { "$ne": config.CONSTANT.ADMIN_TYPE.SUPER_ADMIN } }, { "companyCode": { "$eq": params.companyCode } }] };
			} else {
