...
export class BookingSlotDao extends BaseDao {
    public booking_slot: Model<models.IBookingSlot> = models.booking_slot;

    async bookSlot (params: BookingSlotRequest){
        let query = {_id: params.userId};
        let update = {};
        update["$set"] = {
            isSpotBooked: true,
            bookedSpot: params.spot_id,
            bookedAt: params.bookingRequestedAt
        };
        await this.updateMany("users", query, update, {});
        let obj = {
            userId: params.userId,
            spotId: params.spot_id,
            bookedAt: params.bookingRequestedAt,
            companyId: params.companyId,
            createdAt: Date.now(),
            created:  moment().format('X'),
            companyLocationName: params.companyLocationName,
            floorName: params.floorName,
            zoneName: params.zoneName
        };
        return await this.save("booking_slot", obj);

    }
    async checkTimerForBookingSpot () {
        let query = {"status": {"$ne": config.CONSTANT.STATUS.BLOCKED}, "isSpotBooked": true};
        let users = await this.find("users", query, {bookedAt: 1}, {});
        let current =   parseInt(moment().format('X'));
        for (let user of users){
            if (current - user.bookedAt > 300){
                await this.updateOne("users", {"_id": user._id}, {$set: {"isSpotBooked": false}}, {});
            }
        }
    }
    // async getSuperAdmin() {
    ...
    //     let {pageNo, limit, searchKey, sortBy, sortOrder, fromDate, toDate, requestedPage, userType, compLocationName, status} = params;
    //     let aggPipe = [];
    //     let aggPipe2 = [];
    ...
    // async aggFunc(params, flag, tokenData) {
    //     let {companyLocationName} = params;
    //     let {companyId} = tokenData;
    ...
    //         let matchLocName: any = {};
    //         matchLocName = {$or: [{"users.companyLocationName": companyLocationName}, {"vehicleInfo.companyLocationName": companyLocationName}]};
    //         aggPipe.push({"$match": matchLocName});
    //     }
    //     // let matchLocName: any = {};
