...
export class MailManager {
	...
	async sendMailViaSendgrid(params) {
		try {
			let mailOptions = {
				from: `'Commutev' <${this.fromEmail}>`, // sender email
	...
	async sendAdminForgotPasswordEmail(params) {
		let mailContent = await (new TemplateUtil(config.SERVER.TEMPLATE_PATH + "forgot-password.html"))
			.compileFile({
				...
				"userName": config.SERVER.SUPER_ADMIN_CREDENTIALS.NAME
			});
	...
	async notifiedExpiredCompany(params) {
		let absolutePath = config.SERVER.APP_URL + "/public/image/";
		console.log(JSON.stringify(params) + "absolutePath===============" + absolutePath);
		let mailContent = await (new TemplateUtil(config.SERVER.TEMPLATE_PATH + "expired-company.html"))
			.compileFile({
				"name": params.name,
				"email": params.email,
				"year": new Date().getFullYear(),
		...
		await this.sendMail({ "email": params.email, "subject": config.CONSTANT.EMAIL_TEMPLATE.SUBJECT.EXPIRED_FLEET_LICENCE, "content": mailContent });
	}
}